...
function ExtencaoAPI(extencao, sistema) {
    // Parametros do BD
    ...
    // Initialize
    this.init(extencao, sistema);
}

ExtencaoAPI.prototype.init = function (extencao, sistema) {
    var self = this;
...
ExtencaoAPI.prototype.events = function () {
    ...
    $('body', instancia).on('click','#'+self.id+'-button', function () {
        ...
        label += lista.length<3 ? '' : ' e outros...';
        objeto.val(label);
    });
};
ExtencaoAPI.prototype.accessDatabase = function (url, method, params, sucesso) {
    var rest = new RESTClient();
    rest.setMethod(method);
    rest.setWebServiceURL(url);
    rest.setDataToSender(params);
    rest.setSuccessCallbackFunction(sucesso);
    ...
    self.restClient.setMethod(method);
    self.restClient.setWebServiceURL(url);
    self.restClient.setDataToSender(params);
    self.restClient.setSuccessCallbackFunction(sucesso);
